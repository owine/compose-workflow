...
self-hosted-runner deploy system. Distilled from the `docker-oracleamd`
...
| Placeholder | Meaning | oracleamd example |
|---|---|---|
| `<node>` | short node name / repo suffix | `oracleamd` |
| `<repo>` | caller repo | `docker-oracleamd` |
| `<label>` | runner label (unique per host) | `oracleamd` |
